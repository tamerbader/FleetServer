...
/*------------------------------------
  ...
  2015/09/20  V3.4.8  by Lee  Add support for ARDUINO_MTK_ONE processor.
...
ArduCAM::ArduCAM(byte model ,int CS)
{
	...
	#else
		#if (defined(ARDUINO_MTK_ONE)||defined(ESP32)||defined(TEENSYDUINO))
		  B_CS = CS;
		#else
...
uint8_t ArduCAM:: bus_read(int address)
{
	...
	cbi(P_CS, B_CS);
		...
		return value;	
	#else
		#if (defined(ARDUINO_MTK_ONE) || defined(__arm__) ||defined(TEENSYDUINO))
